<x-admin-app-layout :title="'Employee Project Update'">
    <div class="card card-flash">
        ...
        <div class="card-body pt-0">
            ...
            <form class="form" action="{{ route('admin.employee-project.update', $item->id) }}" method="POST"
                ...
                <div class="row">
                        ...
                        <x-metronic.select-option id="supervisor" required name="supervisor[]" data-hide-search="false"
                            ...
                            @foreach ($admins as $admin)
                                <option value="{{ $admin->id }}"
                                    {{ in_array($admin->id, old('supervisor', json_decode($item->supervisor, true) ?: [])) ? 'selected' : '' }}>
                                    {{ $admin->name }}
                                </option>
                            @endforeach
                    ...
                    <div class="col-lg-3 mb-7">
                        ...
                        <x-metronic.input id="start_time" type="time" name="start_time" :value="old('start_time', $item->start_time)"
                            required></x-metronic.input>
                    </div>
                    ...
                    <div class="col-lg-3 mb-7">
                        ...
                        <x-metronic.input id="end_time" type="time" name="end_time" :value="old('end_time', $item->end_time)"
                            required></x-metronic.input>
                    </div>
                    ...
                    <div class="col-lg-3 mb-7">
                        <x-metronic.label for="type"
                            class="col-form-label fw-bold fs-6">{{ __('Select a Type') }}</x-metronic.label>
                        <x-metronic.select-option id="type" name="type" data-hide-search="true"
                            data-placeholder="Select an option">
                            <option></option>
                            <option value="new" {{ old('type', $item->type) == 'new' ? 'selected' : '' }}>New
                            </option>
                            <option value="update" {{ old('type', $item->type) == 'update' ? 'selected' : '' }}>Update
                            </option>
                            <option value="new_version"
                                {{ old('type', $item->type) == 'new_version' ? 'selected' : '' }}>New Version</option>
                        </x-metronic.select-option>
                    </div>
                    ...
                    <div class="col-lg-3 mb-7">
                        ...
                        <x-metronic.select-option id="project_status" name="project_status" data-hide-search="true"
                            ...
                            <option value="planned"
                                {{ old('project_status', $item->project_status) == 'planned' ? 'selected' : '' }}>
                                Planned</option>
                            <option value="on_going"
                                {{ old('project_status', $item->project_status) == 'on_going' ? 'selected' : '' }}>On
                                Going</option>
                            <option value="completed"
                                {{ old('project_status', $item->project_status) == 'completed' ? 'selected' : '' }}>
                                Completed</option>
                        </x-metronic.select-option>
                    </div>
                    ...
                    <div class="col-lg-3 mb-7">
                        ...
                        <x-metronic.input id="weight" type="number" name="weight" :value="old('weight', $item->weight)"
                            placeholder="Eg:1 to 5"></x-metronic.input>
                    </div>

                    {{-- Kpi Rating --}}
                    <div class="col-lg-3 mb-7">
                        <x-metronic.label for="kpi_rating"
                            class="col-form-label fw-bold fs-6">{{ __('Kpi Rating') }}</x-metronic.label>
                        <x-metronic.input id="kpi_rating" type="number" step="0.01" name="kpi_rating"
                            :value="old('kpi_rating', $item->kpi_rating)" placeholder="Eg:20.00"></x-metronic.input>
                    </div>

                    {{-- Total Working Day --}}
                    <div class="col-lg-3 mb-7">
                        <x-metronic.label for="total_working_day"
                            class="col-form-label fw-bold fs-6">{{ __('Total Working Day') }}</x-metronic.label>
                        <x-metronic.input id="total_working_day" type="number" name="total_working_day"
                            :value="old('total_working_day', $item->total_working_day)" placeholder="Eg:20 Days"></x-metronic.input>
                    </div>
                    ...
                    <div class="col-lg-3 mb-7">
                        <x-metronic.label for="total_working_man_hour"
                            class="col-form-label fw-bold fs-6">{{ __('Total Working Hour') }}</x-metronic.label>
                        <x-metronic.input id="total_working_man_hour" type="number" step="0.01"
                            name="total_working_man_hour" :value="old('total_working_man_hour', $item->total_working_man_hour)" placeholder="10.00"></x-metronic.input>
                    </div>
                    ...
                    {{-- Review --}}
                    <div class="col-lg-12 mb-7">
                        <x-metronic.label for="review"
                            class="col-form-label fw-bold fs-6">{{ __('Review') }}</x-metronic.label>
                        <textarea id="review" class="ckeditor" name="review">{{ old('review', $item->review) }}</textarea>
                    </div>
